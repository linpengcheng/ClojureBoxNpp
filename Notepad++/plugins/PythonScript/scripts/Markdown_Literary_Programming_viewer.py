...
npp_tools_dir = notepad.getNppDir() + "\\tools"
mlt_base_dir = npp_tools_dir + "\\MarkdownLiteraryProgramming"
mlt_out_dir = mlt_base_dir + "\\out"
## mlt_out_dir = mlt_base_dir

cmark_gfm_path = npp_tools_dir + "\\cmark_gfm\\cmark-gfm.exe"
multimarkdown_path = npp_tools_dir + "\\MultiMarkdown\\bin\\multimarkdown.exe"
cutemarked_path = npp_tools_dir + "\\cutemarked\\cutemarked.exe"
...
if viewer == 4:
    ...
    cmd = cmark_gfm_path + " --footnotes " + md_full_path
    r = os.popen(cmd)
    html_text = r.read() 
    html_text = re.sub(r'<pre><code class="language-mermaid">', r'<pre><code class="mermaid">', html_text, 0, re.M)
    # html_text = re.sub('[\r\n\f]{2,}','\r\n', html_text, 0, re.M)
